...
function cellStyle(t, dom) {
  const v = cellVal(t, dom)
  if (!v) return { background: '#14161a', color: '#3a3d43' }
  const colors = ['', '#1e3a2f', '#2f5f3f', '#8a7a2a', '#b5642a', '#c0392b']
  return { background: colors[v], color: '#fff' }
}
function idxColor(i) {
  if (i >= 75) return '#c0392b'
  if (i >= 55) return '#b5642a'
  if (i >= 35) return '#8a7a2a'
  if (i >= 18) return '#2f7f5f'
  return '#2f5f3f'
}
...
<style scoped>
.traj { min-height: 100vh; background: #0f1115; color: #e8eaed; }
.traj__header { display: flex; align-items: center; gap: 16px; padding: 14px 24px; border-bottom: 1px solid #23262c; }
...
.heatmap h3, .branch__section h4 { font-size: 13px; text-transform: uppercase; letter-spacing: .5px; color: #9aa0a6; margin: 0 0 10px; }
.heatmap table { border-collapse: collapse; width: 100%; font-size: 13px; }
.heatmap th { text-align: center; padding: 8px; color: #b8bcc4; border-bottom: 1px solid #2a2d33; }
.heatmap th:first-child, .dom { text-align: left; }
.cell { text-align: center; padding: 8px 10px; font-weight: 600; border: 1px solid #14161a; }
.dom { padding: 8px 10px; color: #cdd0d6; }
.cell--global { font-size: 15px; }
.global-row td { border-top: 2px solid #2a2d33; }

.branches { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; }
.branch { background: #1a1d23; border: 1px solid #2a2d33; border-radius: 10px; padding: 14px; }
...
.branch__idx { color: #fff; font-size: 12px; padding: 2px 8px; border-radius: 10px; }
.branch__hypo { font-size: 12px; color: #9aa0a6; font-style: italic; margin: 0 0 8px; }
.branch__narr { font-size: 13px; color: #cdd0d6; line-height: 1.45; margin: 0 0 10px; }
.branch__section { margin-top: 10px; }
.branch__section ul { margin: 0; padding-left: 16px; font-size: 12px; color: #cdd0d6; }
.branch__section li { margin: 2px 0; }
.decision { margin-bottom: 8px; }
.decision__bar { height: 4px; background: #2a2d33; border-radius: 2px; overflow: hidden; margin-bottom: 3px; }
.decision__fill { height: 100%; }
.decision__txt { font-size: 12px; color: #cdd0d6; }
.decision__type { display: inline-block; width: 15px; height: 15px; line-height: 15px; text-align: center; background: #2a2d33; border-radius: 3px; font-size: 10px; margin-right: 4px; }
.decision__score { color: #9aa0a6; float: right; }
